...
本地开发时，打开浏览器访问：**[http://localhost:8002/web/](http://localhost:8002/web/)**

> **🌐 局域网与广域网访问支持**：  
> 后端网关原生绑定了 `0.0.0.0` 并配置了完全放行的 CORS 策略，**只需确保您的机器/云服务器防火墙开启了 8002 端口**，在同一个局域网下的手机，或者互联网上的任何人都可以直接使用机器的 IP 访问（例如：`http://192.168.x.x:8002/web/`），无需修改任何代码。
